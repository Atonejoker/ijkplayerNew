...
/**
 * Convenience wrapper for av_dict_set() that converts the pointer value to a string
 * and stores it. (ijkplayer extension)
 */
int av_dict_set_intptr(AVDictionary **pm, const char *key, uintptr_t value, int flags);

/**
 * Get a pointer value from dictionary by key. (ijkplayer extension)
 */
uintptr_t av_dict_get_intptr(const AVDictionary *m, const char *key);

/**
 * Convert a hex string to pointer. (ijkplayer extension)
 */
uintptr_t av_dict_strtoptr(const char *value);
