...
static const AVOption options[] = {
    ...
    { "ijkapplication", "AVApplicationContext", OFFSET(app_ctx_intptr), AV_OPT_TYPE_STRING, { .str = NULL }, 0, 0, D },
    { NULL }
};
...
{
    const char *path, *proxy_path, *lower_proto, *local_path;
    ...
    /* ijkplayer: use tcp_hook instead of hardcoded "tcp" */
    lower_proto = s->tcp_hook ? s->tcp_hook : "tcp";
    ...
    if (!strcmp(proto, "https")) {
        /* ijkplayer: disable fastopen for https */
    ...
    if (!s->hd) {
        /* ijkplayer: pass application context to lower protocol */
                     ...
                     AVDictionary **options)
{
    ...
    /* ijkplayer: initialize application context */
    ...
    /* ijkplayer: application callback before HTTP open */
    av_application_will_http_open(s->app_ctx, (void*)h, uri);
    ret = http_open_cnx(h, options);
    /* ijkplayer: application callback after HTTP open */
...
static int64_t http_seek_internal(URLContext *h, int64_t off, int whence, int force_reconnect)
{
    ...
    /* ijkplayer: application callback before HTTP seek */
    av_application_will_http_seek(s->app_ctx, (void*)h, s->location, off);
    if ((ret = http_open_cnx(h, &options)) < 0) {
        /* ijkplayer: application callback after HTTP seek failure */
    ...
    /* ijkplayer: application callback after HTTP seek success */
    av_application_did_http_seek(s->app_ctx, (void*)h, s->location, off, ret, s->http_code);
    av_dict_free(&options);
    ffurl_close(old_hd);
    return off;
