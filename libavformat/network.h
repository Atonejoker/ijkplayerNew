...
/**
 * Send data to a socket with optional Fast Open support.
 * (ijkplayer extension for TCP Fast Open)
 *
 * @param fd         Socket file descriptor.
 * @param msg        Data to send.
 * @param msg_len    Length of data.
 * @param flag       Flags for sendto (can include MSG_FASTOPEN).
 * @param addr       Destination address.
 * @param addrlen    Length of address.
 * @param timeout    Timeout in milliseconds.
 * @param h          URLContext for interrupt callback.
 * @param will_try_next Whether caller will try next address.
 * @return           Number of bytes sent, or AVERROR on failure.
 */
int ff_sendto(int fd, const char *msg, int msg_len, int flag,
              const struct sockaddr *addr, socklen_t addrlen,
              int timeout, URLContext *h, int will_try_next);
