...
static const AVOption options[] = {
    ...
    { "dns_cache_timeout", "dns cache TTL (in microseconds)",   OFFSET(dns_cache_timeout), AV_OPT_TYPE_INT64, { .i64 = 0 },       -1, INT64_MAX, .flags = D|E },
    { "dns_cache_clear", "clear dns cache",   OFFSET(dns_cache_clear), AV_OPT_TYPE_INT, { .i64 = 0},       -1, INT_MAX, .flags = D|E },
    { "fastopen", "enable TCP Fast Open",          OFFSET(fastopen), AV_OPT_TYPE_INT, { .i64 = 0},       0, 1, .flags = D|E },
    { NULL }
};
...
/* ijkplayer: non-blocking DNS resolution */
...
/* ijkplayer: TCP Fast Open support - forward declaration */
...
static int tcp_open(URLContext *h, const char *uri, int flags)
{
    ...
    /* ijkplayer: get application context */
    s->app_ctx = (AVApplicationContext *)av_dict_strtoptr(s->app_ctx_intptr);

    /* ijkplayer: TCP Fast Open - defer connection */
    if (s->fastopen) {
        s->tcp_connected = 0;
        av_strlcpy(s->uri, uri, sizeof(s->uri));
        return 0;
    }
    ...
    if (!dns_entry) {
        ...
        ai = NULL; /* mark that we're using cache, not freeaddrinfo */
    }
...
restart:
    ...
    if (s->listen == 2) {
        ...
        /* ijkplayer: application callback before TCP open */
        ...
        /* Use DNS cache path or parallel connect */
        if (dns_entry) {
            /* When using DNS cache, use sequential connect */
            fd = ff_socket(cur_ai->ai_family,
                           cur_ai->ai_socktype,
                           cur_ai->ai_protocol, h);
            if (fd < 0) {
            ...
            /* Standard parallel connect for non-cached DNS */
        ...
        /* ijkplayer: application callback after TCP open */
        ...
        if (ret) {
            ...
            goto fail1;
        } else if (!dns_entry && s->dns_cache_timeout > 0 && control.ip[0] && !strstr(uri, control.ip)) {
...
static int tcp_read(URLContext *h, uint8_t *buf, int size)
{
    ...
    if (!(h->flags & AVIO_FLAG_NONBLOCK)) {
        ...
        if (ret) {
            /* ijkplayer: custom timeout error code */
            if (ret == AVERROR(ETIMEDOUT)) {
    ...
    if (ret == 0)
        return AVERROR_EOF;
    /* ijkplayer: application callback for traffic monitoring */
    if (ret > 0)
...
static int tcp_write(URLContext *h, const uint8_t *buf, int size)
{
    ...
    /* ijkplayer: TCP Fast Open - deferred connection on first write */
    if (s->fastopen && !s->tcp_connected && size > 0) {
        ret = tcp_fast_open(h, (const char *)buf, s->uri, 0);
        if (!ret) {
            s->tcp_connected = 1;
            if (s->fastopen_success) {
                /* Data was sent with SYN, return success */
                return size;
            }
            /* Fast open fallback, send data normally */
        } else {
            av_log(NULL, AV_LOG_WARNING, "tcp_fast_open error ret = %d\n", ret);
            return ret;
        }
    }
...
/* ijkplayer: TCP Fast Open implementation */
static int tcp_fast_open(URLContext *h, const char *http_request, const char *uri, int flags)
{
    struct addrinfo hints = { 0 }, *ai, *cur_ai;
    int port, fd = -1;
